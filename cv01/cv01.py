        ...
        with open(OUTPUT_FILE, 'a') as f:
            for article in all_articles:
                try:
                    ...
                    json_to_paste = json.dumps(article, ensure_ascii=False, indent=4)
                    f.write(json_to_paste)
                    f.write(',')
                    f.write('\n')
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                content = ' '.join(
                    element.get_text(strip=True) for element in soup.select('div[itemprop="articleBody"] p, div[itemprop="articleBody"] div')
                ) or 'Unknown Content'
                ...
                log_message(f"Chyba při zpracování {url}: {e}", error=True)
                return None

            if not content or content == 'Unknown Content':
                print(f"Chybí obsah článku: {url}")
                log_message(f"Chybí obsah článku: {url}", error=True)
                return None
            
            if not title:
                print(f"Chybí titulek článku: {url}")
                log_message(f"Chybí titulek článku: {url}", error=True)
                return None
